...
@app.before_request
def before_request():
    """ Метод обновляет время последнего запроса пользователя
        и добавляет его в базу как время последнего визита.
    """
    if current_user.is_authenticated:
...
def follow(username):
    ...
    db.session.commit()
    flash('Теперь вы подписаны на пользователя - {}!'.format(username))
    return redirect(url_for('user', username=username))
...
def unfollow(username):
    ...
    db.session.commit()
    flash('Вы больше не подписаны на пользователя - {}!'.format(username))
    return redirect(url_for('user', username=username))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('На указанный e-mail отправлено письмо с инструкциями для завершения сброса пароля')
        return redirect(url_for('login'))
